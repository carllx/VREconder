...
网络共享功能让您可以在局域网内轻松共享 VREconder 处理后的视频，无需使用 U 盘或其他传输工具。

### 典型使用场景

#### 场景 1: VR 头显直接访问 🥽

**问题**：在 PC 上用 VREconder 处理完视频后，需要将文件传输到 VR 头显（如 Oculus Quest、Vive Pro）

**解决方案**：
```
1. 在 PC 上运行 VREconder 处理视频
   ↓
2. 启用网络共享功能
   ↓
3. VR 头显通过 SMB 协议访问共享文件夹
   ↓
4. 直接在头显中播放，无需传输
```
...
**问题**：多台电脑需要访问同一批处理后的 VR 视频

**解决方案**：
```
1. 一台高性能 PC 运行 VREconder 批量处理
   ↓
2. 设置网络共享
   ↓
3. 其他电脑通过网络访问共享文件夹
...
**问题**：需要在平板或手机上查看处理后的视频

**解决方案**：
```
1. PC 上运行 VREconder 处理
   ↓
2. 启用网络共享
   ↓
3. 移动设备通过文件管理器访问共享
...
```
1. 打开文件资源管理器
2. 在地址栏输入: \\<服务器IP>\VR_Project
3. 输入用户名和密码（如果需要）
```

#### VR 头显访问（Oculus Quest 示例）
```
1. 安装 FX File Explorer 或类似应用
2. 添加网络位置
3. 协议: SMB/CIFS
4. 地址: \\<服务器IP>\VR_Project
```
