const input ={
    version: 1,
    mediaFileName: 'Hongou Ai (Nikaidou Yume) - FSVSS001 2.mp4',
    cutSegments: [
      {
        start: 82.99803262647556,
        end: 1100.4562678827995,
        name: '',
      },
    ],
  }
  ...
  Object.keys(segmentsByName).forEach(nameKey => {
    ...
    // Handle null or undefined nameKey by removing the '-' prefix
    let groupVidName = nameKey && nameKey.trim() !== '' ? `${baseVidName}-${nameKey}` : baseVidName;
    if (isDependInit) {
    ...
    if (navigator.platform === 'MacIntel') {
      ...
      groupDATA.dir = `C:\\\\Users\\\\carll\\\\Desktop\\\\${groupVidName}`;
    }
  ...
  // The original global console.log for bash/powershell arrays might not be directly applicable
  // as segments are now grouped. You might want to log this info per group if needed, or remove these global logs.
  // console.log(`Bash:\nstarts=(${starts_bash})\n...`);
  // console.log(`PowerShell: \n$starts=@(${starts_powershell})\n...`);
  
  // The original global download/duration summary:
  // console.log(`Download fragments: ${isDependInit ? fragmentsDownload + 1 : fragmentsDownload}`);
  // console.log(`Exported duration : ${secToTime(exportDuration)} (${exportDuration}) `);
  // These are now logged per group.
